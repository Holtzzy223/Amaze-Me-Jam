...
if(!global.game_pause)
{
	...
	if(current_mission != noone && current_mission._status == STATUS.ACTIVE)
	{
		...
		switch(current_mission._type)
		{
			...
			draw_text(display_get_gui_width()/2,88,string(global.mission_kills)+"	: Destroyed");
			draw_sprite(current_mission._target_sprite,0,display_get_gui_width()/2,48)
			
			break;
			case TYPE.ESCORT:
			draw_text(display_get_gui_width()/2,32,"Locate and Escort");
			draw_sprite(current_mission._target_sprite,0,display_get_gui_width()/2 ,64);
